...
if($_POST["frmFuncao"] == "combinar")
{
	...
	$arMais   = explode(";",  $arLinhas[0]);
	$arOutros = explode(";",	$arLinhas[1]);
	$arMenos  = explode(";",  $arLinhas[2]);
	...
	foreach($arMais as $nrDezena1)
	{
		...
		//
			...
			{
				$arDezena4 = $arOutros; //array_diff($arDezena3, array($nrDezena3));
				foreach($arDezena4 as $nrDezena4)
				{
					$arDezena5 = array_diff($arDezena4, array($nrDezena4));
					foreach($arDezena5 as $nrDezena5)
					{
						$arDezena6 = array_diff($arDezena5, array($nrDezena5)); //$arDezena6 = $arMenos;						
						foreach($arDezena6 as $nrDezena6)
						{
							$arDezena7 = $arMenos; //$arDezena7 = array_diff($arDezena6, array($nrDezena6));
							foreach($arDezena7 as $nrDezena7)
							{
							//
//							echo "$nrDezena1 $nrDezena2 $nrDezena3 $nrDezena4 $nrDezena5 $nrDezena6 <br> \n";
// INICIO JOGO DEFINIDO
									$jogo = array($nrDezena1 ,$nrDezena2, $nrDezena3, $nrDezena4, $nrDezena5, $nrDezena6, $nrDezena7);
									sort($jogo);
									$a = $jogo[0];
									$b = $jogo[1];
									$c = $jogo[2];
...
//									echo "<br> \n cadastrar $a $b $c $d $e $f | ";
									$nrColunas   =           fnConfereColunas7($a, $b, $c, $d, $e, $f, $g);
									$nrLinhas    =            fnConfereLinhas7($a, $b, $c, $d, $e, $f, $g);
									$nrLinha1    =        fnConfereMesmaLinha7($a, $b, $c, $d, $e, $f, $g);
									$nrSequencia =  fnConfereSequenciaSimples7($a, $b, $c, $d, $e, $f, $g);
									$nrMedia     =  array_sum($jogo) / 6;
//									echo $nrColunas.":".$nrLinhas.":".$nrLinha1.":".$nrSequencia;
//									exit();
									// 
									if ( 
											( $nrColunas == 7 and $nrLinhas >= 4 and $nrLinha1 < 3 and $nrSequencia <= 1 and $nrMedia > 20 and $nrMedia < 40 ) OR
											( $nrColunas >= 4 and $nrLinhas >= 3 and $nrLinha1 < 3 and $nrSequencia == 1 and $nrMedia > 20 and $nrMedia < 40 )
										){
										$strSQL = "
											SELECT cd FROM tbl_SenaJogos7 
...
<form method="post">
  ...
  <p>3 linhas 
    <textarea name="numeros" cols="40" rows="3" id="numeros"><?php echo $_POST["mais"];?></textarea>
  </p>
  <p>
